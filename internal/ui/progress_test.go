...
// TestProgressClearBeforeReportOnSharedStream is the ANSI-level contract for
// large directory counts: progress and the report share a TTY cursor. Stop must
// clear the frame before report bytes; post-report clear is the large-dir bug.
func TestProgressClearBeforeReportOnSharedStream(t *testing.T) {
	t.Parallel()
	...
	time.Sleep(40 * time.Millisecond)
	p.Stop()
	_, _ = buf.Write([]byte(reportTail))

	out := buf.String()
	if !strings.Contains(out, reportTail) {
		t.Fatalf("report tail must survive when clear runs first, got %q", out)
	}
	...
	if clearIdx < 0 || reportIdx < 0 || clearIdx > reportIdx {
		t.Fatalf("clear must precede report on shared stream; reportIdx=%d clearIdx=%d out=%q",
